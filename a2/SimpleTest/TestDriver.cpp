        ...
        try {
            ...
            if (errors.empty()) {
                ...
                for (const auto& entry: errors) {
                    ...
                    if (delta > 0) {
                        out << "            " << type << ": Leaked " << pluralize(delta, "allocation") << "." << endl;
                    } else {
                        out << "            " << type << ": Deallocated " << pluralize(-delta, "more time") << " than allocated." << endl;
                    }
                }
...
namespace SimpleTest{
    ...
    void run(TestReporter reporter, TestFilter filter, TestGroupComparator comparator) {
        ...
        for (const auto& entry: Internal::rawTests()) {
            displayedTests.push_back(testsToGroup(entry, filter));
        }
