...
/* Evaluates the given expression. If it throws an exception of the given type, nothing
 * happens. Otherwise, the test fails. For example:
 *
 *     EXPECT_ERROR(v.at(-1), std::out_of_range);
 */
 #define EXPECT_ERROR(condition, type) /* Something internal you shouldn't worry about. */
 ...
 * the student's value and the second should be the correct answer. For example:
 *
 *     EXPECT_NOT_EQUAL(studentValue, referenceValue)
 */
...
namespace SimpleTest {
    namespace Internal {
        ...
        #undef EXPECT_ERROR
        #define EXPECT_ERROR(condition, type) do {\
            try {\
                (void)(condition); \
            } catch (const type& ) { \
                break; \
            ...
            SimpleTest::Internal::doFail("EXPECT_ERROR: " #condition " did not throw an exception of type " #type ".", __LINE__); \
        } while(0)
    }
}
